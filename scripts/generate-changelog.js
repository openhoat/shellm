...
function parseCommitMessage(message) {
  ...
  const match = firstLine.match(/^\[?([A-Z]+)\]?:(.+)/) || firstLine.match(/^(\w+)(?:\([^)]+\))?:(.+)/);
  ...
  const isMajorRefactor = type === 'REFACTOR' && 
    REFACTOR_KEYWORDS.some(keyword => body.includes(keyword) || description.toLowerCase().includes(keyword));
...
function generateChangelogSection(version, date, commits) {
  ...
  if (grouped.REFACTOR.length > 0) {
    ...
    grouped.REFACTOR.forEach(commit => {
      sections.push(`- ${commit.parsed.description}`);
    });
    sections.push('');
  }
